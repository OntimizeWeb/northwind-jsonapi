...
GlobalFilter(
    global_filter_attribute_name="Client_id",  # try customers & categories for u1 vs u2
...
GlobalFilter(
    global_filter_attribute_name="SecurityLevel",  # filters Department 'Eng Area 54'
...
#############################################
GlobalFilter(
    global_filter_attribute_name="Region",  # sales see only Customers in British Isles (9 rows)
    roles_not_filtered=[
        "sa",
        "manager",
        "tenant",
...
GlobalFilter(
    global_filter_attribute_name="Discontinued",  
    roles_not_filtered=["sa", "manager"],  
...
Grant(
    ...
    filter_debug="Customer.Id != 'ANATR'",
)  
# illustrates public role (user p1)
